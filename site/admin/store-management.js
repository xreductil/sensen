(() => {
  ...
  function renderProducts() {
    ...
    $('#product-table').innerHTML = filtered.length ? filtered.map(item => { const pageUrl = item.url || `/product-item/${encodeURIComponent(item.id || '')}/`; return `<tr><td><strong>${escapeHtml(item.title)}</strong><small class="d-block text-secondary">${escapeHtml(item.sku || item.id)}</small></td><td>${escapeHtml(item.cat || '')}</td><td>${escapeHtml(item.spec || '—')}</td><td>${money(item.priceValue)}</td><td class="${Number(item.quantity || 0) < 10 ? 'text-danger fw-bold' : ''}">${Number(item.quantity || 0)}</td><td><span class="badge ${item.published === false ? 'text-bg-secondary' : 'text-bg-success'}">${item.published === false ? '下架' : '上架'}</span></td><td class="text-end"><a class="btn btn-sm btn-outline-success me-1" href="${escapeHtml(pageUrl)}" target="_blank" rel="noreferrer">商品頁</a><button class="btn btn-sm btn-outline-primary me-1" data-edit-product="${escapeHtml(item.id)}">編輯</button><button class="btn btn-sm btn-outline-secondary me-1" data-toggle-product="${escapeHtml(item.id)}">${item.published === false ? '上架' : '下架'}</button><button class="btn btn-sm btn-outline-danger" data-delete-product="${escapeHtml(item.id)}">刪除</button></td></tr>`; }).join('') : '<tr><td colspan="7" class="text-secondary py-4">沒有符合條件的商品。</td></tr>';
